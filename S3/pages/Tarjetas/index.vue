<template>
  <b-content>
    <div id="app" class="d-flex justify-content-center flex-wrap">
      <b-card header="Deudas" header-bg-variant="info" header-text-variant="white">
        <template v-slot:header>
          ...
          <b-button
            style="float:right"
            :to="'/Tarjetas/registrarTarjeta'"
            variant="outline-light"
            v-b-popover.hover.top="'Agregar tarjeta'"
          >
    ...
    <b-modal id="modal-1" hide-footer hide-header no-close-on-backdrop>
      ...
      <b-button
        variant="danger"
        @click="$bvModal.hide('modal-1'), deleteCard(index, idTarjeta)"
      >Eliminar</b-button>
...
export default {
  ...
  methods: {
    ...
    deleteCard(index, id) {
      this.$axios
        .delete(`http://localhost:81/tarjetas/${id}`, id)
        .then(response => {
          console.log(response);
        })
        .catch(error => {
